    ...
    // Failed job goes to "Failed" state (by default) after retries exhausted.
        ...
        protected virtual void TryToCancelOrderPayments(IEnumerable<PaymentIn> toCancelPayments, CustomerOrder order)
        {
            foreach (var payment in toCancelPayments ?? Enumerable.Empty<PaymentIn>())
            {
                ...
                {
                    payment.PaymentMethod?.VoidProcessPayment(new VoidPaymentRequest { Payment = payment, Order = order});
                }
                else if (payment.PaymentStatus == PaymentStatus.Paid)
                {
                    payment.PaymentMethod?.RefundProcessPayment(new RefundPaymentRequest { Payment = payment, Order = order});
                }
                else
                {
