...
namespace VirtoCommerce.OrdersModule.Data.Handlers
{
    ...
    public class AdjustInventoryOrderChangedEventHandler : IEventHandler<OrderChangedEvent>
    {
        ...
        protected virtual string GetFullfilmentCenterForLineItem(LineItem lineItem, Shipment[] orderShipments, List<InventoryInfo> inventoryInfos, Store store)
        {
            ...
            if (string.IsNullOrEmpty(result) && store != null)
            {
                result = inventoryInfos.FirstOrDefault(x => x.ProductId == lineItem.ProductId  &&  x.FulfillmentCenterId == store.MainFulfillmentCenterId && x.InStockQuantity > 0)?.FulfillmentCenterId;
