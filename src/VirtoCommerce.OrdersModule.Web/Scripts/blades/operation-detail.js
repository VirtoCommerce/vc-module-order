angular.module('virtoCommerce.orderModule')
    .controller('virtoCommerce.orderModule.operationDetailController', ['$rootScope', '$scope', 'platformWebApp.dialogService', 'platformWebApp.bladeNavigationService', 'virtoCommerce.orderModule.order_res_customerOrders', 'platformWebApp.objCompareService', '$timeout', 'focus',
        function ($rootScope, $scope, dialogService, bladeNavigationService, customerOrders, objCompareService, $timeout, focus) {
                                ...
                                if (remove) {
                                    if (blade.id === 'operationDetail') {
                                        ...
                                        customerOrders.delete({ ids: blade.customerOrder.id }, function () {
                                            blade.parentBlade.refresh();
                                            bladeNavigationService.closeBlade(blade);
                                            $rootScope.$emit('orderEvent:delete', blade.customerOrder); // only triggers $rootScope.$on() events
                                        });
                                    }
                                }
                            }
